...
const chartOption = computed(() => ({
  ...
  series: [{
    name: activeTab.value === 'expense' ? '支出' : '收入',
    type: 'pie',
    radius: ['32%', '60%'],
    center: ['50%', '42%'],
    label: {
...
async function handleSubmitEntry(form: { type: string; amount: number; date: string }) {
  ...
  try {
    ...
    if (kind === 'expense') {
      ...
      entrySuccess.value = res?.warning ? `新增成功（提醒：${res.warning}）` : '新增成功'
    } else {
    ...
    entryError.value = err instanceof Error ? err.message : '新增失敗'
  } finally {
...
async function fetchLedger() {
  ...
  try {
    ...
    ledgerError.value = err instanceof Error ? err.message : '取得紀錄失敗'
  } finally {
...
async function handleEditItem(item: LedgerEntry, draft: { date: string; amount: number }) {
  try {
    ...
    ledgerError.value = err instanceof Error ? err.message : '更新失敗'
  }
}
...
async function handleDeleteItem(item: LedgerEntry) {
  try {
    ...
    ledgerError.value = err instanceof Error ? err.message : '刪除失敗'
  }
}
...
async function fetchReport() {
  ...
  try {
    ...
    reportError.value = err instanceof Error ? err.message : '取得報表失敗'
  } finally {
